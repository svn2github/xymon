...
\chapter{HOBBIT-CLIENTS.CFG}

\section{HOBBIT-CLIENTS.CFG}
 Section: File Formats (5) 
Updated: Version Exp: 11 Jan 2008 
Index Return to Main Contents 
 \section{NAME}
 hobbit-clients.cfg - Configuration file for the hobbitd\_client module 
...
\section{FILE FORMAT}
 ...
  
\section{CPU STATUS COLUMN SETTINGS}
...
\end{description}
 ...
  
\section{DISK STATUS COLUMN SETTINGS}
 ...
  
\section{MEMORY STATUS COLUMN SETTINGS}
...
\end{description}
 ...
  
\section{PROCS STATUS COLUMN SETTINGS}
...
    PROC ''\%hobbitd\_channel --channel=data.*hobbitd\_rrd'' 1 1 yellow 
...
       Note that no processes are checked by default. 
 ...
  
\section{MSGS STATUS COLUMN SETTINGS}
...
       color=''red'', no ``excludepattern''. 
 ...
  
\section{FILES STATUS COLUMN SETTINGS}
...
\end{description}
 ...
  
\section{PORTS STATUS COLUMN SETTINGS}
...
        PORT LOCAL=\%[.:]22\$ STATE=ESTABLISHED MAX=5 TRACK=ssh TEXT=SSH 
 ...
  
\section{CHANGING THE DEFAULT SETTINGS}
...
\section{RULES TO SELECT HOSTS}
 ...
 Rule matching an alert by the name of the page in BB. ``targetstring'' is the path of the page as defined in the bb-hosts file. E.g. if you have this setup: \begin{description}
\item[]\begin{verbatim}
...
, e.g. \textbf{PAGE=/}
 would match all hosts on the Hobbit frontpage. If you need it in a regular expression, use \textbf{PAGE=\%\^{}/}
 to avoid matching the forward-slash present in subpage-names. 
 ...
 Rule match by the client class-name. You specify the class-name for a host when starting the client through the ``--class=NAME'' option to the runclient.sh script. If no class is specified, the host by default goes into a class named by the operating system. 
 ...
  
\section{DIRECTING ALERTS TO GROUPS}
 ...
  
\section{RULES: APPLYING SETTINGS TO SELECTED HOSTS}
...
\end{description}
  ...
  If you have multiple settings that you want to apply the same rules to, you can write the rules *only* on one line, followed by the settings. E.g. \begin{description}
\item[]\begin{verbatim}
...
\end{description}
  ...
  will apply the three settings to all of the ``db'' hosts on week-days between 8AM and 4PM. This can be combined with per-settings rule, in which case the per-settings rule overrides the general rule; e.g. \begin{description}
\item[]\begin{verbatim}
...
\end{description}
 ...
  
\section{NOTES}
 For the LOG, FILE and DIR checks, it is necessary also to configure the actual file- and directory-names in the \emph{client-local.cfg(5)}
 file. If the filenames are not listed there, the clients will not collect any data about these files/directories, and the settings in the hobbit-clients.cfg file will be silently ignored. 
...
\item[DESCRIPTION]
\item[FILE FORMAT]
\item[CPU STATUS COLUMN SETTINGS]
\item[DISK STATUS COLUMN SETTINGS]
\item[MEMORY STATUS COLUMN SETTINGS]
\item[PROCS STATUS COLUMN SETTINGS]
\item[MSGS STATUS COLUMN SETTINGS]
\item[FILES STATUS COLUMN SETTINGS]
\item[PORTS STATUS COLUMN SETTINGS]
\item[CHANGING THE DEFAULT SETTINGS]
\item[RULES TO SELECT HOSTS]
\item[DIRECTING ALERTS TO GROUPS]
\item[RULES: APPLYING SETTINGS TO SELECTED HOSTS]
\item[NOTES]
\item[SEE ALSO]
