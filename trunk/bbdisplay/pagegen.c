	...
	if (groupcols || (maxbanksize > 0)) {
		...
		for (h = head; (h); h = h->next) {
			...
			if (h->pretitle || (h == head)) {
				...
				if ((groupcols == NULL) && (maxbanksize > 0)) {
					...
					for (i=0; (i < maxbanksize); i++) {
						fprintf(output, " <TD ALIGN=CENTER VALIGN=BOTTOM WIDTH=20>");
						fprintf(output, " <FONT %s><B>%d</B></FONT></TD>\n", 
							getenv("MKBBCOLFONT"), i);
					}
				}
				else {
			...
			if ((groupcols == NULL) && (h->banksize > 0)) {
				...
				for (i=0; (i < h->banksize); i++) {
					sprintf(alttag, "%s:%s", u32toIP(baseip+i), colorname(h->banks[i]));
					...
					fprintf(output, "<IMG SRC=\"%s/%s\" ALT=\"%s\" HEIGHT=\"%s\" WIDTH=\"%s\" BORDER=0></A>",
						bbskin, dotgiffilename(h->banks[i], 0, 0),
...
void do_summaries(dispsummary_t *sums, FILE *output)
{
	...
	for (s=sums; (s); s = s->next) {
		...
		if (newhost == NULL) {
			/* New summary "host" */
			newhost = init_host(s->row, NULL, NULL, 0,0,0,0, 0, 0, 0.0, NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, 0);
