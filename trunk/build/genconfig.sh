...
$MAKE -C build test-socklent 1>/dev/null 2>&1
if test $? -eq 0; then
...
$MAKE -C build test-snprintf 1>/dev/null 2>&1
if test $? -eq 0; then
...
$MAKE -C build test-vsnprintf 1>/dev/null 2>&1
if test $? -eq 0; then
...
$MAKE -C build test-rpcenth 1>/dev/null 2>&1
if test $? -eq 0; then
