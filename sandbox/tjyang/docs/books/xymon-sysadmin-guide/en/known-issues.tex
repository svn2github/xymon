\chapter{Known issues in xymon}

\section{Known issues in xymon}
...
\item xymon fails on FreeBSD with ``Could not get sem: No space left on device''
\item xymon will not run inside a FreeBSD jail
\item xymon on Solaris compiles but aborts with some ``ld.so'' error
\item xymon on Solaris compiles but aborts with some ``ld.so'' error
\item xymon on Solaris 10 ``xymond status-board not available''

\end{itemize}
\subsubsection{How to report bugs}


 If you think you have found a bug in xymon, please report it to the Xymon mailing list at xymon@hswn.dk. You can do a lot to help getting bugs fixed by providing detailed information about the bug:
\begin{itemize}
\item Always include the version number of xymon you're using
\item If one of the xymon tools crashes and leaves a core-file (usually in the ~xymon/server/tmp/ directory), please use the \textbf{gdb}
 tool to pinpoint where the crash occurred:\\ 
\begin{itemize}
\item Login as the xymon user
...
\subsubsection{xymon fails on FreeBSD with ``Could not get sem: No space left on device''}
...
\subsubsection{xymon will not run inside a FreeBSD jail}
 ...
 sysctl that must be enabled. If it isn't then xymon will refuse to start with the error Could not get shm of size 262144: Function not implemented.
\subsubsection{xymon on Solaris compiles but aborts with some ``ld.so'' error}
...
\subsubsection{xymon on Solaris 10 ``xymond status-board not available''}


 It seems that xymon triggers a bug in the Solaris 10 ``TCP fusion'' loopback interface, resulting in no data being returned from xymond when queried for the current status of all monitored systems. The recommended solution is to disable the use of TCP fusion. Colin Spargo writes:
\begin{verbatim}
...
but will require xymon to be restarted. To do this:
