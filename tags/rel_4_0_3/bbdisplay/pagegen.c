		...
		for (h = head; (h); h = h->next) {
			...
			for (gc = groupcols; (gc); gc = gc->next) {
				...
				if (e == NULL) {
					...
					else if (genstatic && strcmp(e->column->name, xgetenv("INFOCOLUMN")) && strcmp(e->column->name, xgetenv("LARRDCOLUMN"))) {
						/*
						 * Dont use htmlextension here - it's for the
						 * pages generated by bbd.
						 * We dont do static pages for the info- and larrd-columns, because
